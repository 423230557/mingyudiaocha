...
class ListView
{
    ...
    function __construct($typeid, $uppage=1)
    {
        ...
        if(!$this->IsError)
        {
			...
			if(!preg_match("#^http:\/\/#i", $this->Fields['content']))
			{
				$pattern_tmp = str_replace('/', '\/', $GLOBALS['cfg_cmspath']);
				if (!preg_match("#^{$pattern_tmp}#i", $this->Fields['content'])) {
					$pregRule = "/<img(.*?)src(\s*)=(\s*)[\'|\"]\/(.*?(?:[\.jpg|\.jpeg|\.png|\.gif|\.bmp|\.ico]))[\'|\"](.*?)[\/]?(\s*)>/i";
					$this->Fields['content'] = preg_replace($pregRule, '<img ${1} src="'.$GLOBALS['cfg_cmspath'].'/${4}" ${5} />', $this->Fields['content']);
				}
			}
			// --------end
			if(!preg_match("#^http:\/\/#i", $this->TypeLink->TypeInfos['content']))
			{
				$pattern_tmp = str_replace('/', '\/', $GLOBALS['cfg_cmspath']);
				if (!preg_match("#^{$pattern_tmp}#i", $this->TypeLink->TypeInfos['content'])) {
					$pregRule = "/<img(.*?)src(\s*)=(\s*)[\'|\"]\/(.*?(?:[\.jpg|\.jpeg|\.png|\.gif|\.bmp|\.ico]))[\'|\"](.*?)[\/]?(\s*)>/i";
					$this->TypeLink->TypeInfos['content'] = preg_replace($pregRule, '<img ${1} src="'.$GLOBALS['cfg_cmspath'].'/${4}" ${5} />', $this->TypeLink->TypeInfos['content']);
				}
			}
			//----end
        ...
        for($i=0;$i<$row;$i++)
        {
            ...
            for($j=0;$j<$col;$j++)
            {
                if($row = $this->dsql->GetArray("al"))
                {
                    ...
                    {
                        $pattern_tmp = str_replace('/', '\/', $GLOBALS['cfg_cmspath']);
                        if (!preg_match("#^{$pattern_tmp}#i", $row['litpic'])) {
                            $row['litpic'] = $GLOBALS['cfg_cmspath'].$row['litpic'];
                            $row['litpic'] = str_replace('//', '/', $row['litpic']);
                        }
                    }
					// --------end
